...
define(['jquery'], function() {
	 ...
	 */
		...
		if (!/android|iPad|iPod|iPhone|arm/.test(navigator.platform)) {
			hasTouch = false;
		} else {
	 ...
	 */
		...
		window.jQuery.tableDnD = {
			...
			*/
				$(table).on(startEvent, 'tr ' + config.dragHandle, function(e) {
					$.tableDnD.initialiseDrag($(this), table, this, e, config);
					//return false;
				});
			},
			...
			initialiseDrag: function(dragObject, table, target, e, config) {
				this.dragObject = dragObject.closest('tr')[0];
				this.dragCell = dragObject.closest('th')[0];
				this.currentTable = table;
				this.columns = this.currentTable.tHead.childNodes[0].cells;
				this.mouseOffset = this.getMouseOffset(target, e);
				this.originalOrder = this.currentOrder();

				// Call the onDragStart method if there is one
				if (config.onDragStart && !config.onDragStart(table, target)) {
					return;
				}
		...
		To do this we need the element's position and the mouse position */
			...
			moveHorizontal: function(moving, currentCell) {
				var config = this.currentTable.tableDnDConfig,
					i,
					j;
				...
				var from = this.dragCell.cellIndex;
				var to = currentCell.cellIndex;
				...
				if (from < to) {
					for (i = from; i < to; ++i) {
						for (j = 0; j < this.currentTable.rows.length; ++j) {
							td1 = $(this.currentTable.rows[j]).find(j === 0 ? "th" : "td")[i];
							if (td1 && !$(td1).hasClass('dndUnswappable')) {
								td2 = td1.nextSibling;
								swapNodes(td1, td2);
							}
						}
					...
					for (i = from; i > to; --i) {
						for (j = 0; j < this.currentTable.rows.length; ++j) {
							td1 = $(this.currentTable.rows[j]).find(j === 0 ? "th" : "td")[i - 1];
							if (td1 && !$(td1).hasClass('dndUnswappable')) {
								td2 = td1.nextSibling;
								swapNodes(td1, td2);
							}
			...
			getTableOffset: function() {
				return {
					x: $(this.currentTable).offset().left,
					y: $(this.currentTable).offset().top
				};
			},
			mousemove: function(e) {
				var dragObj = $($.tableDnD.dragObject),
					dragCell = $.tableDnD.dragCell,
				...
				if (!dragCell) {
					...
					config.onDragClass && $(dragCell).addClass(config.onDragClass);
				}
				...
				if (!dragCell) {
					...
					var offset = $.tableDnD.getTableOffset();
				...
				if (dragCell) currentColumn = $.tableDnD.findDropTargetCell(dragCell, x, moving);


				$.tableDnD.moveVerticle(moving, currentRow);
				$.tableDnD.moveHorizontal(moving, currentColumn);
			...
			/** We're only worried about the x position really, because we can only move columns left and right */
			findDropTargetCell: function(draggedCell, x, moving) {
				var cellWidth,
				...
				for (var i = 0; i < columns.length; i++) {
					cell = columns[i];
					cellX = this.getPosition(cell).x;
					cellWidth = cell.offsetWidth;

					// Because we always have to insert before, we need to offset the height a bit
					if (draggedCell.offsetWidth >= cellWidth) {
						...
						// If it's the same as the current row, ignore it
							if (cell == draggedCell
								|| cell.cellIndex === 0
								|| (config.onAllowDrop
									&& !config.onAllowDrop(window.draggedRow, row))
								// If a row has nodrop class, then don't allow dropping (inspired by John Tarr and Famic)
								|| $(cell).hasClass("nodrop"))
								return null;
							else
								return cell;
					} else {
						if (moving.horizontal > 0) {
							if ((x > cellX) && (x < cellX + draggedCell.offsetWidth))
							// that's the row we're over
							// If it's the same as the current row, ignore it
								if (cell == draggedCell
									|| cell.cellIndex === 0
									...
									return cell;
						} else if (moving.horizontal < 0) {
							if ((x > cellX + cellWidth - draggedCell.offsetWidth) && (x < cellX + cellWidth))
							// that's the row we're over
							// If it's the same as the current row, ignore it
								if (cell == draggedCell
									|| cell.cellIndex === 0
			...
			processMouseup: function() {
				...
				var config = this.currentTable.tableDnDConfig,
					...
					droppedCell = this.dragCell,
					parentLevel = 0,
					myLevel = 0;

				if (!droppedRow && !droppedCell) return null;
				...
				config.onDrop
					//  && this.originalOrder != this.currentOrder()
					&& $(droppedRow).hide().fadeIn('fast', function() {
						config.onDrop(this.currentTable, droppedRow, droppedCell);
					});
